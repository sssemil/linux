...
int get_resource(struct otg_dev *dev)
{
	 ...
	 * get pmu vbus addr.
	 */
	ret = of_property_read_u32(base_dev->of_node, "pmu_vbus_reg_addr", &(dev->pmu_vbus_reg_addr));
	if (ret) {
		usb_err("get pmu_vbus_reg_addr failed !!! \n");
		dev->pmu_vbus_reg_addr = 0x6;
	}

	/*
	 * get pmu vbus offset.
	 */
	ret = of_property_read_u32(base_dev->of_node, "pmu_vbus_reg_offset", &(dev->pmu_vbus_reg_offset));
	if (ret) {
		usb_err("get pmu_vbus_reg_offset failed !!! \n");
		dev->pmu_vbus_reg_offset = 0x1;
	}
...
static int is_usb_cable_connected(void)
{
	...
	if (!dev->fpga_flag) {
		/*lint -e713 -e701*/
		ret = (hisi_pmic_reg_read(dev->pmu_vbus_reg_addr) & (1 << dev->pmu_vbus_reg_offset));
		/*lint +e713 +e701*/
	}
	return ret;
}
