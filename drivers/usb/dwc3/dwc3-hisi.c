...
{
	unsigned int pmic_status0 = hisi_pmic_reg_read(hisi_dwc3_dev->pmu_vbus_reg_addr);/*lint !e713*/
	return !!(pmic_status0 & (1 << hisi_dwc3_dev->pmu_vbus_reg_offset));/*lint !e701*/
}
...
static int get_resource(struct hisi_dwc3_device *hisi_dwc3)
{
	 ...
	 * get pmu vbus addr.
	 */
	if (of_property_read_u32(dev->of_node, "pmu_vbus_reg_addr", &(hisi_dwc3->pmu_vbus_reg_addr))) {
		usb_err("get pmu_vbus_reg_addr failed !!! \n");
		hisi_dwc3->pmu_vbus_reg_addr = PMU_VBUS_REG_DEFAULT_ADDR;
	}

	/*
	 * get pmu vbus offset.
	 */
	if (of_property_read_u32(dev->of_node, "pmu_vbus_reg_offset", &(hisi_dwc3->pmu_vbus_reg_offset))) {
		usb_err("get pmu_vbus_reg_offset failed !!! \n");
		hisi_dwc3->pmu_vbus_reg_offset = PMU_VBUS_REG_DEFAULT_OFFSET;
	}
