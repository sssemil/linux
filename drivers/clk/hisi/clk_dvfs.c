...
struct peri_dvfs_clk {
	...
	unsigned int			sensitive_volt[DVFS_MAX_FREQ_NUM+1];
	u32					sensitive_level;
	u32					block_mode;
	u32					div;
	u32					div_bits;
...
static void updata_freq_volt_up_fn(struct work_struct *work)
{
	...
	unsigned long target_freq = sw->target_freq;
	unsigned long target_volt = sw->target_volt;
	struct peri_volt_poll *pvp = sw->pvp;
	struct clk *friend_clk = sw->linkage;
...
static void __init hisi_peri_dvfs_clk_setup(struct device_node *np)
{
	...
	u32 sensitive_volt[DVFS_MAX_FREQ_NUM+1] = {0};
