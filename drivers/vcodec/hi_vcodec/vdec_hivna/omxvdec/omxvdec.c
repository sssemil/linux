...
static HI_S32 omxvdec_compat_get_data(COMPAT_TYPE_E eType, HI_VOID __user *pUser, HI_VOID *pData)
{
	...
	case T_SM_CONFIG: {
		...
		ret |= copy_from_user(pIoctlCfg, pCompatCfg, sizeof(COMPAT_OMXSCD_REG_CFG_S));
...
static long omxvdec_compat_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	case VDEC_IOCTL_GET_OPEN_COUNT:
		...
		ret = copy_to_user(vdec_msg.out, &(omxvdec->open_count), sizeof(HI_U32));
		if (ret != HI_SUCCESS) {
	...
	case VDEC_IOCTL_POWER_CTRL:
		ret = copy_from_user(&power_enable, vdec_msg.in, sizeof(HI_U32));
		if (ret != HI_SUCCESS) {
			OmxPrint(OMX_FATAL, "VDEC_IOCTL_POWER_CTRL : copy_from_user failed!\n");
			return -EIO;
		}
	...
	case VDEC_IOCTL_SET_CLK_RATE:
		ret = copy_from_user(&clk_rate, vdec_msg.in, sizeof(CLK_RATE_E));
		if (ret != HI_SUCCESS) {
		...
		if (VDEC_Regulator_SetClkRate(clk_rate) != HI_SUCCESS) {
			OmxPrint(OMX_FATAL,"VDEC_IOCTL_SET_CLK_RATE : VDEC_Regulator_SetClkRate faied!\n");
			return HI_FAILURE;
		}
		break;
	...
	case VDEC_IOCTL_GET_CLK_RATE:
		if (VDEC_Regulator_GetClkRate(&clk_rate) == HI_SUCCESS) {
			ret = copy_to_user(vdec_msg.out, &clk_rate, sizeof(CLK_RATE_E));
			if (ret != HI_SUCCESS) {
				OmxPrint(OMX_FATAL, "VDEC_IOCTL_GET_CLK_RATE: VDEC_Regulator_GetClkRate failed!\n");
				return -EIO;
			}
		} else {
			OmxPrint(OMX_FATAL,"VDEC_IOCTL_GET_CLK_RATE : VDEC_Regulator_GetClkRate faied!\n");
			return HI_FAILURE;
		}
		break;

	case VDEC_IOCTL_VDM_PROC:
		ret = copy_from_user(&vdm_reg_cfg, vdec_msg.in, sizeof(OMXVDH_REG_CFG_S));
		if (ret != HI_SUCCESS) {
		...
		if (ret != HI_SUCCESS) {
			OmxPrint(OMX_FATAL, "VDEC_IOCTL_VDM_PROC : VCTRL_VDMHal_Process failed!\n");
			return -EIO;
		}
		ret = copy_to_user(vdec_msg.out, &vdm_state_reg, sizeof(VDMHAL_BACKUP_S));
		if (ret != HI_SUCCESS) {
	...
	case VDEC_IOCTL_GET_VDM_HWSTATE:
		...
		ret = copy_to_user(vdec_msg.out, &vdm_is_run, sizeof(HI_S32));
		if (ret != HI_SUCCESS) {
	...
	case VDEC_IOCTL_GET_DTS_CONFIG:
		if (VFMW_GetDtsConfig(&dts_config) == VDEC_OK) {
			ret = copy_to_user(vdec_msg.out, &dts_config, sizeof(VFMW_DTS_CONFIG_S));
			if (ret != HI_SUCCESS) {
				OmxPrint(OMX_FATAL, "VDEC_IOCTL_GET_DTS_CONFIG : copy_to_user failed!\n");
				return -EIO;
			}
		} else {
			OmxPrint(OMX_FATAL, "VDEC_IOCTL_GET_DTS_CONFIG : VFMW_GetDtsConfig failed!\n");
			return VDEC_ERR;
		}
		break;
	...
	case VDEC_IOCTL_SCD_PROC:
		...
		if (ret != HI_SUCCESS) {
			OmxPrint(OMX_FATAL, "VDEC_IOCTL_SCD_PROC : omxvdec_SMHal_Process failed!\n");
			return -EIO;
		}

		ret = copy_to_user(vdec_msg.out, &scd_state_reg, sizeof(SCD_STATE_REG_S));
		if (ret != HI_SUCCESS) {
	...
	case VDEC_IOCTL_SET_BPD_PROC:
		ret = copy_from_user(&bpd_reg, vdec_msg.in, sizeof(OMXBPD_REG_S));
		if (ret != HI_SUCCESS) {
		...
		if (ret != HI_SUCCESS) {
			OmxPrint(OMX_FATAL, "VDEC_IOCTL_SET_BPD_PROC :  VCTRL_BPDHal_Process failed!\n");
			return -EIO;
		}
		ret = copy_to_user(vdec_msg.out, &bpd_reg, sizeof(OMXBPD_REG_S));
		if (ret != HI_SUCCESS) {
