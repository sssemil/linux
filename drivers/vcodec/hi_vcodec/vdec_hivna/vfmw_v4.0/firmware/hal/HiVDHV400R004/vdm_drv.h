...
typedef struct
{
	...
	SINT32 VdhAvsFlag;//REG_AVS_FLAG;
	SINT32 VdhSelRst; //REG_VDH_SELRST;
	SINT32 VdhBasicCfg0;//VREG_BASIC_CFG0
	SINT32 VdhBasicCfg1;//VREG_BASIC_CFG1
	SINT32 VdhAvmAddr;//VREG_AVM_ADDR
	SINT32 VdhVamAddr; //VREG_VAM_ADDR
	SINT32 VdhStreamBaseAddr;//VREG_STREAM_BASE_ADDR
	SINT32 VdhEmarId;//REG_EMAR_ID//需要读一次，根据读取的值做判断再写
	SINT32 VdhSedTo;//VREG_SED_TO
	SINT32 VdhItransTo;//VREG_ITRANS_TO
	SINT32 VdhPmvTo;//VREG_PMV_TO
	SINT32 VdhPrcTo;//VREG_PRC_TO
	SINT32 VdhRcnTo;//VREG_RCN_TO
	SINT32 VdhDblkTo;//VREG_DBLK_TO
	SINT32 VdhPpfdTo;//VREG_PPFD_TO
	SINT32 VdhPartLevel;//VREG_PART_DEC_OVER_INT_LEVEL
	SINT32 VdhYstAddr;//VREG_YSTADDR_1D
	SINT32 VdhYstride;//VREG_YSTRIDE_1D
	SINT32 VdhUvoffset;//VREG_UVOFFSET_1D、
	SINT32 VdhHeadInfOffset;//VREG_HEAD_INF_OFFSET
	SINT32 VdhLineNumAddr;//VREG_LINE_NUM_STADDR//只有H264和HEVC需要写
	SINT32 VdhPpfdBufAddr;//VREG_PPFD_BUF_ADDR//H264、HEVC、VP6、VP8需要写
	SINT32 VdhPpfdBufLen;//VREG_PPFD_BUF_LEN//H264、HEVC、VP6、VP8需要写
	SINT32 VdhRefPicType;//VREG_REF_PIC_TYPE//AVS、DIVX3、H264、MP2、RV8、RV9、VC1需要写
	SINT32 VdhFfAptEn;//VREG_FF_APT_EN//VP8不需要写
	SINT32 VdhIntState; //VREG_INT_STATE
	SINT32 VdhIntMask;
	SINT32 LowlyEnable;
