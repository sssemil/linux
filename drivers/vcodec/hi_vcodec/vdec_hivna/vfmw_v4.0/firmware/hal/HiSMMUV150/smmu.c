...
static SINT32 alloc_smmu_tlb_miss_addr(VOID)
{
    ...
    ret = MEM_AllocMemBlock_2("SMMU_RDERR", SMMU_RWERRADDR_SIZE, &gAllocMem_RD, 0);
    if (ret != MEM_MAN_OK)
    {
    ...
    ret = MEM_AllocMemBlock_2("SMMU_WRERR", SMMU_RWERRADDR_SIZE, &gAllocMem_WR, 0);
    if (ret != MEM_MAN_OK)
    {
        dprint(PRN_FATAL, "%s kzalloc mem for smmu wrerr failed!\n", __func__);
        MEM_ReleaseMemBlock_2(&gAllocMem_RD);
        return SMMU_ERR;
    }
#endif
    return SMMU_OK;
...
VOID SMMU_InitGlobalReg(VOID)
{
    ...
    if (1 == gSmmuInitFlag)
    {
        ...
        {
            set_common_reg(SMMU_SMRx_NS + i*0x4, 0xC, 32, 0);//0x00000003 none secure
        }
        for (i = 1; i < SMRx_ID_SIZE; i += 2)
        {
            set_common_reg(SMMU_SMRx_NS + i*0x4, 0xD, 32, 0);//0x00000002 secure
        }
...
VOID SMMU_DeInit(VOID)
{
    ...
    {
        MEM_ReleaseMemBlock_2(&gAllocMem_WR);
    }
#endif
}
