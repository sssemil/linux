...
VOID SCDDRV_WriteReg(SM_CTRLREG_S *pSmCtrlReg)
{
    ...
    PrintScdVtrlReg();
    WR_SCDREG(REG_SCD_PREVIOUS_BYTE_LSB, pSmCtrlReg->ScdPreviousByteLsb);
    WR_SCDREG(REG_SCD_PREVIOUS_BYTE_MSB, pSmCtrlReg->ScdPreviousByteMsb);
...
VOID SCDDRV_SaveStateReg(VOID)
{
    ...
    gScdStateReg.ScdIntFlag = RD_SCDREG(REG_SCD_INT_FLAG);
    ...
    gScdStateReg.ScdPreviousByteMsb = RD_SCDREG(REG_SCD_PREVIOUS_BYTE_MSB);
    gScdStateReg.ScdPreviousByteLsb = RD_SCDREG(REG_SCD_PREVIOUS_BYTE_LSB);
    gScdStateReg.ScdByteValid = RD_SCDREG(REG_SCD_BYTE_VALID);
}
...
VOID SCDDRV_ISR( VOID )
{
	...
	dat = RD_SCDREG(REG_SCD_INT_FLAG) & 0x01;
	if((dat & 1) == 0)
    {
...
SINT32 WaitSCDFinish(VOID)
{
    ...
    if ( 1 == s_SCDState )
    {
        ...
        {
            if ( (RD_SCDREG( REG_SCD_INT_FLAG ) & 1) )
            {
                return FMW_OK;
            }
            else
...
static VOID PrintScdVtrlReg(VOID)
{
    ...
    SmCtrlReg.ScdPreviousByteLsb = RD_SCDREG(REG_SCD_PREVIOUS_BYTE_LSB);
    SmCtrlReg.ScdPreviousByteMsb = RD_SCDREG(REG_SCD_PREVIOUS_BYTE_MSB);
    ...
    dprint(PRN_SCD_REGMSG, "ScdPreviousByteLsb = %x\n", SmCtrlReg.ScdPreviousByteLsb);
    dprint(PRN_SCD_REGMSG, "ScdPreviousByteMsb = %x\n", SmCtrlReg.ScdPreviousByteMsb);
