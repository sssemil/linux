...
SINT32 SOS_MapAddrCheck(UINT64 Phy, UINT32 Size, UINT32 IsNormalMem)
{
    ...
    if ((1 == IsNormalMem) || ((0 == IsNormalMem) && ((Phy >= HEAP_SEC_DRM_BASE) && ((Phy + Size) > HEAP_SEC_DRM_BASE) && ((Phy + Size) <= (HEAP_SEC_DRM_BASE + HEAP_SEC_DRM_SIZE)))))
    {
        return OSAL_OK;
    }
    else
...
SINT8* SOS_MapShare(UINT64 Phy, UINT32 Size, UINT32 IsNormalMem, UINT32 IsCached)
{
    ...
    //dprint(PRN_ALWS, "%s mmap Phy=%x, Size=%x, IsNormalMem=%d, IsCached=%d, reutrn Vir=%pK\n", __func__, Phy, Size, IsNormalMem, IsCached, (SINT8 *)VirAddr);
...
SINT32 SOS_UnMapShare(VOID *Vir, UINT32 Size)
{
    ...
    //dprint(PRN_ALWS, "%s unmap Vir=%pK, Size=%x\n", __func__, Vir, Size);
    if (NULL == Vir || 0 == Size)
    {
