...
#define    MAX_MEM_MAN_RECORD_NUM    (MAX_CHAN_NUM*32)  //MAX_CHAN_NUM*5 Ã¿¸öÍ¨µÀÎªADD 11 ´Î
    ...
    for (i=0; i<MAX_MEM_MAN_RECORD_NUM; i++)
    {
        if ((s_MemRecord[i].PhyAddr <= PhyAddr) && (PhyAddr < s_MemRecord[i].PhyAddr + s_MemRecord[i].Length))
        {
            dprint(PRN_ALWS, "%s ERROR: Phy [0x%x ~ 0x%x] overlap with Record[%d] [0x%x ~ 0x%x]\n", __func__,
                   PhyAddr, PhyAddr+Length, i, s_MemRecord[i].PhyAddr, s_MemRecord[i].PhyAddr+s_MemRecord[i].Length);
            IsErrorFlag = 1;
            break;
        }
...
SINT32  MEM_MapRegisterAddr(UADDR RegStartPhyAddr, UINT32 RegByteLen, MEM_RECORD_S *pMemRecord)
{
    ...
    if (NULL == pMemRecord || 0 == RegStartPhyAddr || 0 == RegByteLen || NULL == VFMW_OSAL_RegisterMap)
    {
        return MEM_MAN_ERR;
    }
...
{
    if (0 == PhyAddr || 0 == VirAddr || NULL == VFMW_OSAL_RegisterUnMap)
    {
        return;
    }
