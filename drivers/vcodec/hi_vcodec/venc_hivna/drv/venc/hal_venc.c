...
{
	pVeduReg->SMMU_ERR_RDADDR = g_smmu_err_mem.RdAddr;//config alloc phyaddr,in order system don't dump
	pVeduReg->SMMU_ERR_WRADDR[0] = g_smmu_err_mem.WrAddr;
}
...
HI_VOID VeduHal_CfgReg_LambdaSet(VENC_REG_INFO_S *channelcfg)
{
	...
	HI_INFO_VENC("**********************VeduHal_LambdaSet\n");
...
HI_VOID VeduHal_CfgReg_SMMUSet(VENC_REG_INFO_S *channelcfg)
{
	...
	HI_INFO_VENC("phy_pgd_base:0x%lx\n",phy_pgd_base);
	HI_INFO_VENC("(phy_pgd_base >> 32):0x%x\n",(phy_pgd_base >> 32));
	//SMRX_0
...
HI_VOID VeduHal_CfgReg_PREMMUSet(VENC_REG_INFO_S *channelcfg)
{
	...
	pAllReg = (S_HEVC_AVC_REGS_TYPE*)VeduIpCtx.pRegBase;
	HI_INFO_VENC("MMU RRE ADDR enter %s\n", __func__);
	HI_INFO_VENC("%s,ADDR--MMU_PRE_NBI_MVST_ADDR_STR=%x,pAllReg->SMMU_MSTR_SMRX_1[0]=%x,pAllReg->MMU_PRE_STRMADDR0_STR=%x\n", __func__,(&(pAllReg->MMU_PRE_NBI_MVST_ADDR_STR)),(&(pAllReg->
					SMMU_MSTR_SMRX_1[0])),(&(pAllReg->MMU_PRE_STRMADDR0_STR)));
	HI_INFO_VENC("%s,ADDR--MMU_PRE_NBI_MVST_ADDR_STR=%x,pAllReg->SMMU_MSTR_SMRX_1[0]=%x,pAllReg->MMU_PRE_STRMADDR0_STR=%x\n", __func__,(&(pAllReg->MMU_PRE_NBI_MVST_ADDR_STR)),(&(pAllReg->
					SMMU_MSTR_SMRX_1[0])),(&(pAllReg->MMU_PRE_STRMADDR0_STR)));

	HI_INFO_VENC("%s,ADDR--MMU_PRE_NBI_MVST_ADDR_STR=%x,pAllReg->SMMU_MSTR_SMRX_2[0]=%x,pAllReg->MMU_PRE_STRMADDR0_STR=%x\n", __func__,(&(pAllReg->MMU_PRE_NBI_MVST_ADDR_STR)),(&(pAllReg->
