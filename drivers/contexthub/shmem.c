...
static void shmem_callback(void __iomem * buf_addr, unsigned int buf_size)
{
	...
	if (NULL == buf_addr)
		return;
	show_buff = memcmp(buf_addr, t_pool_send, (unsigned long)udata_size);
	do_gettimeofday(&tv);
				   ...
				   size_t count, loff_t * ppos)
{
	...
	udata_size = val;
	memset_s((void *)t_pool_send, sizeof(t_pool_send), 0xaa, (size_t)val);
	do_gettimeofday(&tv);
	pr_err("S:tv_sec[%ld]tv_usec[%06ld]\n", tv.tv_sec, tv.tv_usec);
	ret = shmem_send(TAG_SHAREMEM, (const void *)t_pool_send, (unsigned int)val);
	if (ret)
