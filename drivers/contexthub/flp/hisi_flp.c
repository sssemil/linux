...
/*lint -e650 -e834 -e440 -e702 -e679 -esym(440,*) -esym(702,*) -esym(679,*) */
    for (row = 0; left; row++) {
...
#else
static void flp_hex_dump(char *buf, int size, char per_row)/*lint -e715*/
{
    return;
}
#endif
        ...
        unsigned int count, unsigned char cmd_type)
{
	...
	flp_hex_dump(data, (int)count, 32);
	...
	context_iomcu_cfg_t *cur, context_iomcu_cfg_t *old_set)
{
	...
	for(i = 0;i < context_max; i++) {
		if(old_set->context_list[i].head.event_type) {
			...
			if(old_set->context_list[i].head.len) {
...
static int get_ar_data_from_mcu(const pkt_header_t *head)
{
	...
	flp_hex_dump((char *)head, (int)(sizeof(pkt_header_t) + head->length), 32);
	if (list_empty(&g_flp_dev.list)) {
		pr_err("[%s] no app\n", __func__);
		return -EFAULT;
	}
...
#if SAVE_PDR_DATA_TO_FTRACE
            trace_flp_ar_event(pd->context_num, pevent->event_type, pevent->context, pevent->msec, pevent->buf_len,
			flp_port->port_type, flp_port->channel_type, flp_port->flushing,port_etype,head->length);
#endif
			if((port_etype & pevent->event_type) ||
				...
				if (AR_VER_1 == flp_port->ar_ver) {
					...
					printk(HISI_FLP_DEBUG"type[%u]ctx[%u]msec[%llu]len[%u]\n",
						pevent->event_type, pevent->context, pevent->msec, pevent->buf_len);
					if (pevent->buf_len) {
						flp_hex_dump(pevent->buf, (int)pevent->buf_len, 32);
					}
				}else {
...
#if SAVE_PDR_DATA_TO_FTRACE
            trace_flp_env_event(pd->context_num, pevent->event_type, pevent->context, pevent->msec, pevent->buf_len,
			flp_port->port_type, flp_port->channel_type,port_etype);
#endif
			if(port_etype & pevent->event_type)
				copy_data_to_buf(pdata, (char *)pevent,
...
void flp_netlink_send_packet (void)
{
    ...
    list_for_each(pos, &g_flp_dev.list) {
        ...
        flp_generate_netlink_packet(flp_port, (char *)&packet,
            (unsigned int)sizeof(flp_pdr_data_t), 0);
    }
}
...
static int flp_pdr_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd & FLP_IOCTL_CMD_MASK) {
        ...
        case FLP_IOCTL_PDR_WRITE(0):
            ...
            return pdr_write_cmd(flp_port, (char __user *)para.buf, para.len);
