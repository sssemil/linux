...
static void mmc_bkops_start_stop_statistic(int start, u64 start_time, struct mmc_card *card)
{
	u64 stop_time;
	u64 time_interval;
	struct mmc_bkops_stats *bkops_stats_p = &(card->bkops_stats);
	...
	stop_time = ktime_get_ns();
	time_interval = stop_time - start_time;
	if ( BLK_BKOPS_STOP == start && (time_interval > bkops_stats_p->bkops_max_stop_time) ) {
	...
	} else if ( BLK_BKOPS_START == start && (time_interval > bkops_stats_p->bkops_max_start_time) ) {
...
put_card:
	...
	if (!ret) {
		mmc_bkops_start_stop_statistic(start, start_time, card);
	}
...
struct blk_bkops_ops mmc_bkops_ops = {
	.bkops_idle_start_interval_ms = 1000,
