...
int check_batt_not_exist(char *buf)
{
	...
	if (!is_hisi_battery_exist()) {
		snprintf(buf, DSM_BATTERY_MAX_SIZE, "batt_temp = %d\n", hisi_battery_temperature());
		return 1;
	}
#endif
	return 0;
...
int check_batt_volt_overhigh(char *buf)
{
	...
	if (volt > BATT_VOLT_OVERHIGH_TH) {
		...
		snprintf(buf, DSM_BATTERY_MAX_SIZE, "batt_volt = %dmV, capacity = %d, curr = %dmA, avg_curr = %dmA\n",
...
int check_batt_terminate_too_early(char *buf)
{
	...
	if (di->chargedone_stat && capacity <= CHG_CANT_FULL_THRESHOLD) {
		...
		avg_curr = hisi_battery_current_avg();
		rm = hisi_battery_rm();
		snprintf(buf, DSM_BATTERY_MAX_SIZE, "battery: %s, batt_volt = %dmV, batt_curr = %dmA, "
...
int check_vbus_volt_overhigh(char *buf)
{
	...
	if (times == DSM_CHECK_TIMES_LV2) {
		times = 0;
		vbatt = hisi_battery_voltage();
		snprintf(buf, DSM_BATTERY_MAX_SIZE, "vbus_status= %s, vbatt = %dmV\n",
...
int check_charge_temp_fault(char *buf)
{
	...
	if ((HIGH_TEMP < temp || LOW_TEMP > temp) && (di->usb_online || di->ac_online)) {
		snprintf(buf, DSM_BATTERY_MAX_SIZE, "temp fault so not charge temp = %d\n", temp);
		return 1;
	}
#endif
	return 0;
...
int check_charge_batt_capacity(char *buf)
{
	...
	if ((VCHRG_CHARGE_DONE_EVENT == di->event) && full_rm < fcc_design*80/100) {
			snprintf(buf, DSM_BATTERY_MAX_SIZE, "battery: %s, batt_vol = %dmV, capacity = %dmAh, full capacity = %dmAh \n",
