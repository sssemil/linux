...
static int calculate_state_of_charge(struct smartstar_coul_device *di)
{
    ...
    if(g_basp_full_pc)
    {
         soc = soc*PERMILLAGE/g_basp_full_pc;
         hwlog_info("update soc=%d\n", soc);
    }
    if (soc > 100)
    	soc = 100;
        ...
        for (i = 0; i < di->basp_total_level; i++) {
            if (di->basp_level == basp_policy[i].level) {
                if (basp_policy[i].err_no) {
                    basp_dsm_flag = charger_dsm_report(basp_policy[i].err_no, (int *)&di->basp_level);
                }
                break;
            }
        }
...
static void readjust_fcc_table(struct smartstar_coul_device *di)
{
	...
	{
		temp->x[i] = now->x[i];
		ratio = div_u64(((u64)(now->y[i]) * 1000), fcc);/*lint !e574*/
...
{
    int ret = 0;
    ret = of_property_read_u32(np, "low_temp_opt_enable", &low_temp_opt_flag);
