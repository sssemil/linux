...
int dpe_set_clk_rate(struct platform_device *pdev)
{
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970) {
		ret = clk_set_rate(hisifd->dss_mmbuf_clk, pdss_clk_rate->dss_mmbuf_rate);
		if (ret < 0) {
			HISI_FB_ERR("fb%d dss_mmbuf clk_set_rate(%llu) failed, error=%d!\n",
				hisifd->index, pdss_clk_rate->dss_mmbuf_rate, ret);
			return -EINVAL;
		}
	}
	...
	if (hisifd->index == PRIMARY_PANEL_IDX) {
		if (g_dss_version_tag == FB_ACCEL_KIRIN970) {
			HISI_FB_INFO("dss_mmbuf_clk:[%llu]->[%llu].\n",
				pdss_clk_rate->dss_mmbuf_rate, (uint64_t)clk_get_rate(hisifd->dss_mmbuf_clk));
		}
...
static void dss_lp_set_reg(char __iomem *dss_base)
{
	...
	outp32(dss_base + DSS_WCH0_DMA_OFFSET + DSS_WCH0_FBCE_CREG_CTRL_GATE,  0x00000000);
	outp32(dss_base + DSS_WCH1_DMA_OFFSET + DSS_WCH1_FBCE_CREG_CTRL_GATE,  0x00000000);
	...
	outp32(dss_base + DSS_WCH0_DMA_OFFSET + DSS_WCH0_FBCE_CREG_CTRL_GATE,  0x0000000c);
	outp32(dss_base + DSS_WCH1_DMA_OFFSET + CH_CLK_SEL, 0x00000000);
	outp32(dss_base + DSS_WCH1_DMA_OFFSET + DSS_WCH1_FBCE_CREG_CTRL_GATE,  0x0000000c);
