...
void hisi_dss_csc_init(char __iomem *csc_base, dss_csc_t *s_csc)
{
	...
	s_csc->icg_module = inp32(csc_base + CSC_ICG_MODULE_ES);
	...
	char __iomem *csc_base, dss_csc_t *s_csc)
{
	...
	hisifd->set_reg(hisifd, csc_base + CSC_ICG_MODULE_ES, s_csc->icg_module, 32, 0);
...
void hisi_dss_dpp_acm_gm_set_reg(struct hisi_fb_data_type *hisifd) {
	...
	if (0 == gamma_config_flag) {
		if (last_gamma_type != hisifd->panel_info.gamma_type) {
			...
			//disable xcc
			set_reg(xcc_base + XCC_BYPASS, 0x1, 1, 0);
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970_ES) {
		...
		//enable xcc
		set_reg(xcc_base + XCC_BYPASS, 0x0, 1, 0);
	}
