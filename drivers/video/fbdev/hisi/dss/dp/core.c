...
void dptx_global_intr_en(struct dp_ctrl *dptx)
{
	...
	outp32(dptx->base + DPTX_WRAP_REG_DPC_INT_MASK, 0x3);

	/* Enable all HPD interrupts */
	hpd_ien = inp32(dptx->base + DPTX_HPD_IEN);
	hpd_ien |= (DPTX_HPD_IEN_IRQ_EN |
		    DPTX_HPD_IEN_HOT_PLUG_EN |
		    DPTX_HPD_IEN_HOT_UNPLUG_EN);
	outp32(dptx->base + DPTX_HPD_IEN, hpd_ien);
#if HDCP_ENABLE
	/* Mask interrupt related to HDCP22 GPIO output status */
	reg = inp32(dptx->base + DPTX_HDCP_API_INT_MSK);
	reg |= DPTX_HDCP22_GPIOINT;
	outp32(dptx->base + DPTX_HDCP_API_INT_MSK, reg);
#endif
}
...
void dptx_global_intr_dis(struct dp_ctrl *dptx)
{
	...
	outp32(dptx->base + DPTX_WRAP_REG_DPC_INT_MASK, 0x0);
...
{
	outp32(dptx->base + DPTX_ISTS, DPTX_ISTS_ALL_INTR);

	/*  all HPD interrupts */
	outp32(dptx->base + DPTX_HPDSTS, (DPTX_HPDSTS_IRQ |DPTX_HPDSTS_HOT_PLUG | DPTX_HPDSTS_HOT_UNPLUG));
}
...
void dptx_aux_disreset(struct dp_ctrl *dptx, bool enable)
{
	uint32_t reg;
	/* Enable AUX Block */
	reg = (uint32_t)inp32(dptx->base + DPTX_WRAP_REG_DPC_AUX_CTRL);//lint !e838
	...
	outp32(dptx->base + DPTX_WRAP_REG_DPC_AUX_CTRL, reg);
...
void dptx_soft_reset(struct dp_ctrl *dptx, uint32_t bits)
{
	...
	/* Set reset bits */
	rst = inp32(dptx->base + DPTX_SRST_CTRL);
	rst |= bits;
	outp32(dptx->base + DPTX_SRST_CTRL, rst);
...
bool dptx_check_dptx_id(struct dp_ctrl *dptx)
{
	...
	dptx_id = inp32(dptx->base + DPTX_ID);
...
int dptx_core_init(struct dp_ctrl *dptx)
{
	...
	outp32(dptx->base + DPTX_CCTL, 0x0);

	/* Check the core version */
	memset(str, 0, sizeof(str));
	version = inp32(dptx->base + DPTX_VER_NUMBER);
	...
	version = inp32(dptx->base + DPTX_VER_TYPE);
	str[5] = '-';
