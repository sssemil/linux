...
void dptx_audio_sdp_en(struct dp_ctrl *dptx)
{
	...
	reg = (uint32_t)inp32(dptx->base + DPTX_SDP_VERTICAL_CTRL);
	reg |= DPTX_EN_AUDIO_STREAM_SDP;
	outp32(dptx->base + DPTX_SDP_VERTICAL_CTRL, reg);

	reg = (uint32_t)inp32(dptx->base + DPTX_SDP_HORIZONTAL_CTRL);
	reg |= DPTX_EN_AUDIO_STREAM_SDP;
	outp32(dptx->base + DPTX_SDP_HORIZONTAL_CTRL, reg);
}
...
void dptx_audio_infoframe_sdp_send(struct dp_ctrl *dptx)
{
	...
	outp32(dptx->base + DPTX_SDP_BANK, audio_infoframe_header);
	outp32(dptx->base + DPTX_SDP_BANK + 4, audio_infoframe_data[0]);
	outp32(dptx->base + DPTX_SDP_BANK + 8, audio_infoframe_data[1]);
	outp32(dptx->base + DPTX_SDP_BANK + 12, audio_infoframe_data[2]);
...
void dptx_en_audio_channel(struct dp_ctrl *dptx, int ch_num, int enable)
{
	...
	reg = (uint32_t)inp32(dptx->base + DPTX_AUD_CONFIG1);
	...
	outp32(dptx->base + DPTX_AUD_CONFIG1, reg);
}
...
void dptx_video_reset(struct dp_ctrl *dptx, int enable)
{
	...
	reg = (uint32_t)inp32(dptx->base + DPTX_SRST_CTRL);
	if (enable)
		reg |= DPTX_SRST_VIDEO_RESET;
	else
		reg &= ~DPTX_SRST_VIDEO_RESET;
	outp32(dptx->base + DPTX_SRST_CTRL, reg);
}
...
void dptx_audio_samp_freq_config(struct dp_ctrl *dptx)
{
	...
	aparams = &dptx->aparams;
	reg = (uint32_t)inp32(dptx->base + DPTX_AG_CONFIG4);
	...
	outp32(dptx->base + DPTX_AG_CONFIG4, reg);
}
...
void dptx_audio_gen_config(struct dp_ctrl *dptx)
{
	...
	/* AG_CONFIG3 */
	reg = (uint32_t)inp32(dptx->base + DPTX_AG_CONFIG3);
	...
	outp32(dptx->base + DPTX_AG_CONFIG3, reg);
	/* AG_CONFIG4 */
	...
	/* AG_CONFIG5 */
	reg = (uint32_t)inp32(dptx->base + DPTX_AG_CONFIG5);
	reg &= ~DPTX_AG_CONFIG3_CH_NUMCL0_MASK;
	...
	outp32(dptx->base + DPTX_AG_CONFIG5, reg);
}
...
void dptx_video_timing_change(struct dp_ctrl *dptx)
{
	...
	outp32(dptx->base + DPTX_VG_SWRST, 0);
}
...
void dptx_video_core_config(struct dp_ctrl *dptx)
{
	...
	outp32(dptx->base + DPTX_VSAMPLE_POLARITY_CTRL, reg);
	...
	outp32(dptx->base + DPTX_VIDEO_CONFIG3, reg);
	...
	outp32(dptx->base + DPTX_VIDEO_CONFIG4, reg);
	...
	outp32(dptx->base + DPTX_VIDEO_MSA1, reg);
...
void dptx_video_ts_change(struct dp_ctrl *dptx)
{
	...
	reg = (uint32_t)inp32(dptx->base + DPTX_VIDEO_CONFIG5);
	...
	outp32(dptx->base + DPTX_VIDEO_CONFIG5, reg);
}
...
void dptx_video_set_core_bpc(struct dp_ctrl *dptx)
{
	...
	reg = inp32(dptx->base + DPTX_VSAMPLE_CTRL);
	...
	outp32(dptx->base + DPTX_VSAMPLE_CTRL, reg);
}
...
void dptx_video_set_sink_col(struct dp_ctrl *dptx)
{
	...
	reg_msa2 = inp32(dptx->base + DPTX_VIDEO_MSA2);
	...
	outp32(dptx->base + DPTX_VIDEO_MSA2, reg_msa2);
}
...
void dptx_video_set_sink_bpc(struct dp_ctrl *dptx)
{
	...
	reg_msa2 = inp32(dptx->base + DPTX_VIDEO_MSA2);
	reg_msa3 = inp32(dptx->base + DPTX_VIDEO_MSA3);
	...
	outp32(dptx->base + DPTX_VIDEO_MSA2, reg_msa2);
	outp32(dptx->base + DPTX_VIDEO_MSA3, reg_msa3);
...
void dptx_disable_default_video_stream(struct dp_ctrl *dptx)
{
	...
	vsamplectrl = inp32(dptx->base + DPTX_VSAMPLE_CTRL);
	vsamplectrl &= ~DPTX_VSAMPLE_CTRL_STREAM_EN;
	outp32(dptx->base + DPTX_VSAMPLE_CTRL, vsamplectrl);
}
