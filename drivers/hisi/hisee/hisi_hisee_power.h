...
/* 0x5 means power off status
 * 0xa means power on status
 */
typedef union {
	unsigned int value;
	struct {
		unsigned int huawei_wallet:4;
		unsigned int u_shield:4;
		unsigned int time_out:4;
		unsigned int power_upgrade:4;
		unsigned int chip_test:4;
