...
static bool s_imx376_bac_power_on = false;/*false: power off, true:power on*/
extern struct hw_csi_pad hw_csi_pad;
//#define POWER_SETTING_DELAY_1 1//now POWER_SETTING_DELAY_1 is not used
...
struct sensor_power_setting hw_imx376_bac_power_setting[] = {
...
atomic_t volatile imx376_powered = ATOMIC_INIT(0);/*init*/
sensor_t s_imx376_bac =
{
    .intf = { .vtbl = &s_imx376_vtbl, },
    .power_setting_array = {
            .size = ARRAY_SIZE(hw_imx376_bac_power_setting),
            .power_setting = hw_imx376_bac_power_setting,
     },
...
{
    {
        .compatible = "huawei,imx376_bac",
        .data = &s_imx376_bac.intf,
    },
    {
    },
};
...
int imx376_power_up(hwsensor_intf_t* si)
{
    ...
    if (hw_is_fpga_board()) {
         cam_info("%s. enter hw_is_fpga_board.", __func__);
        ret = do_sensor_power_on(sensor->board_info->sensor_index, sensor->board_info->name);
    } else {
         cam_info("%s. enter hw_sensor_power_up.", __func__);
        ...
        struct platform_device* pdev)
{
    ...
    if (pdev->dev.of_node) {
        rc = hw_sensor_get_dt_data(pdev, &s_imx376_bac);
        if (rc < 0) {
    ...
    s_imx376_bac.dev = &pdev->dev;

    rc = hwsensor_register(pdev, &s_imx376_bac.intf);
    if (rc < 0)
    {
        hwsensor_unregister(&s_imx376_bac.intf);
        cam_err("%s hwsensor_register fail.\n", __func__);
        goto imx376_sensor_probe_fail;
    }
    rc = rpmsg_sensor_register(pdev, (void*)&s_imx376_bac);
    if (rc < 0)
    {
        rpmsg_sensor_unregister((void*)&s_imx376_bac);
        cam_err("%s rpmsg_sensor_register fail.\n", __func__);
		goto imx376_sensor_probe_fail;
    }
    cam_info("exit %s",__func__);
...
/*lint -e528 -esym(528,*)*/
module_init(imx376_bac_init_module);
module_exit(imx376_bac_exit_module);
/*lint -e528 +esym(528,*)*/
/*lint -e753 -esym(753,*)*/
MODULE_DESCRIPTION("imx376_bac");
