...
int dump_jpeg_reg(void __iomem * viraddr)
{
    ...
    for (i = 0 ; i <= JPGENC_FORCE_CLK_ON_CFG_REG;) {
        cam_info("%s: jpeg reg 0x%x = 0x%x", __func__, i, get_reg_val((void __iomem*)((char*)viraddr + i)));
        i += 4;
    }
    for (i = JPGENC_DBG_0_REG ; i <= JPGENC_DBG_13_REG;) {
        cam_info("%s: dbg reg 0x%x = 0x%x", __func__, i, get_reg_val((void __iomem*)((char*)viraddr + i)));
        i += 4;
    }
    return 0;
}
