			...
			struct timeval *pstop_tv)
{
    ...
    if(NULL == pstart_tv || pstop_tv)
    {
        vl53l0_errmsg("input param is NULL");
        return;
    }
...
static int stmvl53l0_laser_get_set_cal_data(struct stmvl53l0_data* data, hwlaser_calibration_data_t *p)
{
    ...
    {
        rc = papi_func_tbl->GetOffsetCalibrationDataMicroMeter(data, offset);
        if(rc != VL53L0_ERROR_NONE)
        {
            vl53l0_errmsg("get offset cal fail!!");
        }
        rc = papi_func_tbl->GetXTalkCompensationRateMegaCps(data, xtalk);
        if(rc != VL53L0_ERROR_NONE)
        {
            vl53l0_errmsg("get xtalk fail!!");
        }
    ...
    switch(param->name) {
        case (DEVICEMODE_PAR):
            if (param->is_read) {
                ...
                data,
                (VL53L0_DeviceModes *)&(param->value));
            } else {
                param->status =
...
#else
    rc = 0;
    vl53l0_dbgmsg("stub request irq");
#endif
