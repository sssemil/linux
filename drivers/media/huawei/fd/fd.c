...
static long AHFD_Wait_Result(struct fd_device *pdev, unsigned long arg)
{
    ...
    fd_info("AHFD_WAIT_RESULT type = %d\n", waitType);
    ...
    if(waitType & RESULT_TYPE_PRE)
    {
        ...
        fd_info("AHFD_WAIT_RESULT IDX_WAIT_PRE finished, status = %ld\n", rc);
    }
    ...
    if(waitType & RESULT_TYPE_TME)
    {
        ...
        fd_info("AHFD_WAIT_RESULT IDX_WAIT_TME finished, status = %ld\n", rc);
      // fd_info("mask 0x%08x, flags 0x%08x", g_mask, g_flags);
       total_time = PRE_time+TME_time;
       //fd_info("pre time is %lu \n", (unsigned long)PRE_time);
       //fd_info("TME time is %lu \n", (unsigned long)TME_time);
       //fd_info("total_time  is %lu \n", (unsigned long)total_time);
    }
    return rc;
}
